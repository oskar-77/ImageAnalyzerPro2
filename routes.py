...
def upload_file():
    ...
    try:
        ...
        if file and allowed_file(file.filename):
            ...
            flash('File type not allowed. Please upload PNG, JPG, JPEG, GIF, BMP, TIFF, or WEBP files.', 'error')
            return redirect(url_for('index'))
...
def compare_images():
    ...
    try:
        ...
        # Validate images
        if not (ImageUtils.validate_image(filepath1) and ImageUtils.validate_image(filepath2)):
            os.remove(filepath1)
            os.remove(filepath2)
    ...
    try:
        ...
        if report_format == 'txt':
            with open(report_path, 'w') as f:
                ...
                f.write("STATISTICS\n")
                f.write("-" * 20 + "\n")
                f.write(f"Mean Brightness: {statistics_data['brightness']['mean']:.2f}\n")
                f.write(f"Standard Deviation: {statistics_data['brightness']['std']:.2f}\n")
                ...
                if 'channel_stats' in statistics_data:
                    f.write("\nCHANNEL STATISTICS\n")
                    f.write("-" * 20 + "\n")
                    for channel, data in statistics_data['channel_stats'].items():
                        f.write(f"{channel}: Mean={data['mean']:.2f}, Std={data['std']:.2f}\n")
            ...
            # Statistics text
            ax3.text(0.1, 0.9, f"Mean Brightness: {statistics_data['brightness']['mean']:.2f}", transform=ax3.transAxes, fontsize=12)
            ax3.text(0.1, 0.8, f"Std Deviation: {statistics_data['brightness']['std']:.2f}", transform=ax3.transAxes, fontsize=12)
            ax3.text(0.1, 0.7, f"Min Value: {statistics_data['brightness']['min']}", transform=ax3.transAxes, fontsize=12)
            ax3.text(0.1, 0.6, f"Max Value: {statistics_data['brightness']['max']}", transform=ax3.transAxes, fontsize=12)
